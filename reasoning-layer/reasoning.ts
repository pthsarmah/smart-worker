...
import { logger, logAIInteraction } from "../logger";
...
function extractErrorSignature(
    ...
    const errorMatch =
        ERROR_TYPE_RE.exec(stacktrace) || ERROR_TYPE_RE.exec(failedReason || "");

    const errorType = errorMatch?.groups?.type || "Error";
    const errorMessage =
        errorMatch?.groups?.message || failedReason || "Unknown error";
...
export async function extractStructuredFailureContext(
    job: Job
): Promise<StructuredFailureContext> {
    ...
    const snippetPromises = failureLocations
        .slice(0, 3)
        .map((loc) => extractFocusedSnippet(loc.filePath, loc.lineNumber));
    const snippetResults = await Promise.all(snippetPromises);
    const focusedSnippets = snippetResults.filter(
        (s): s is FocusedCodeSnippet => s !== null
    );
...
FAILURE LOCATIONS
==================
${structuredContext.failureLocations
        .map(
            (loc, i) =>
...
`;
    ...
    const focusedSnippetsContext = structuredContext.focusedSnippets
        .map(
            (snippet, i) =>
...
Output: One short resolution summary paragraph.
    ...
    try {
        const reasonText = await aiClient.getChatContent(messages);
        stopSpinner();
        return JSON.stringify(reasonText);
    } catch (e) {
        logger.error({ error: e }, "Error generating resolution summary");
        stopSpinner();
        return;
    }
};
...
export const jobFailureReasoning = async (job: Job) => {
    const { jobContext, codeContext, structuredContext, focusedSnippetsContext } =
        await getStacktracePathsCodeContext(job);
    var prompt =
        jobContext +
        focusedSnippetsContext +
        "\n\n==================\nFULL CODE CONTEXT\n==================\n" +
    ...
    const { electionResults, resolutionSummary, meanDistance, signatureMatch } =
        await searchJobFromMemory(structuredContext);
    stopSpinner();
    ...
    if (electionResults && resolutionSummary) {
        ...
        logger.info(
            {
                winnerJobId: electionResults.winner,
                matchType,
                similarityPercent: ((1 - meanDistance) * 100).toFixed(2),
                meanDistance: meanDistance.toFixed(4),
            },
            "Found similar job in memory"
        );
        prompt =
            `
===================================================
...
===================================================
	${resolutionSummary}
        ...
        logger.info("No similar jobs found in memory");
    }
...
`.trim(),
    ...
    try {
        reasonText = await aiClient.getChatContent(messages);
        stopSpinner();
    } catch (e) {
        logger.error({ error: e }, "Error connecting with LLM");
        stopSpinner();
        return;
    }
    ...
    if (!rootDir) {
        logger.error("No root directory found in configuration");
        return;
    }

    const codeChanges: CodeChange[] = await Promise.all(
        Array.from(JSON.stringify(reasonText).matchAll(TS_CAPTURE_REGEX)).map(
            async (match) => {
...
`;
    ...
    if (codeChanges.length > 0) {
        logger.info({ changeCount: codeChanges.length }, "Fix generated, testing in Docker sandbox");
        ...
        if (result) {
            ...
            logger.info({ summary }, "Resolution summary generated");
            await storeJobToMemory(
                job,
                structuredContext,
                result,
                summary as string
